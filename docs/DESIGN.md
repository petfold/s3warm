...
12. **s3warm in the browser (distant).** With in-browser Swarm nodes maturing, could the gateway itself compile to Wasm and run installation-free? The architecture is favorable: the core is an `http.Handler` (no listener needed — a Service Worker intercepting `fetch()` to a virtual endpoint feeds it synthetic requests), the Bee client is outbound-only (`net/http` on `js/wasm` already rides on `fetch`; an in-browser node needs only an injected transport shim), and everything except the SQLite driver compiles for `js/wasm` today (verified). The index has two paths: in-memory rebuilt from the bucket's feed/commit chain at startup (§5 recovery — the most Swarm-native answer), or a browser-persistent store — swarmlite already runs SQLite-WASM in the browser over a custom Swarm VFS, so a SQLite-WASM/OPFS-backed `Store` bridged over `syscall/js` is proven ground. The client that makes it worthwhile is aws-sdk-js: existing S3 web apps migrating to Swarm with an endpoint swap and zero installation. Open sub-questions: Go Wasm binary size (~10–20 MB), keeper/committer behavior under tab and Service-Worker lifetimes (flush on `beforeunload`), streaming upload support in `fetch`, and postage bought from a browser wallet instead of the node wallet.
